...
export default function TimeTracker() {
    ...
    (async () => {
      try {
        ...
        if (active) {
          ...
          if (active.coach) setCoachName(active.coach);
        }
      } catch (e) {
  ...
  function startSegment(type, coach) {
    const id = `${Date.now()}`;
    const entry = { id, type, start: new Date().toISOString(), end: null, ...(coach ? { coach } : {}) };
  ...
  const statusMeta = {
    ...
    coaching: { label: `Coaching · ${coachName}`, sub: activeEntry ? `Since ${fmtTime(activeEntry.start)}` : "" },
  }[status];
      ...
      <div style={{ maxWidth: 480, margin: "0 auto" }}>
        ...
        {entries.length > 0 && (
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 12, color: COLORS.slate, marginBottom: 6, fontWeight: 600 }}>TODAY'S TIMELINE</div>
              ...
              >
                <div>
                  <div style={{ fontSize: 14, fontWeight: 600 }}>
                    {e.type === "coaching" ? `Coaching — ${e.coach}` : SEGMENT_LABEL[e.type]}
                  </div>
      ...
      {showCoachModal && (
        ...
        >
          ...
          >
            ...
            <label style={{ fontSize: 12, color: COLORS.slate, fontWeight: 600 }}>COACH'S NAME</label>
            <input
              autoFocus
              value={coachName}
              onChange={(e) => setCoachName(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleCoachSubmit()}
              placeholder="e.g. Mansfield Eyre"
              style={{
                width: "100%",
                marginTop: 6,
                padding: "10px 12px",
...
function ActionButton({ icon, label, onClick, disabled, color, dark }) {
  return (
    <button
      className="btn"
      onClick={onClick}
